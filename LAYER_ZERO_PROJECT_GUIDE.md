...
- 매크로 실행
- 콘솔 명령 프리셋 + 수동 입력
- 자동레벨링: `베드 50°C 가열 → G28 → BED_MESH_CALIBRATE → SAVE_CONFIG`
- 자동레벨링 진행 단계(5단계) 실시간 표시
...
- 정비 체크리스트 및 주기 관리
- 실패 증상 트리아지
- 베드 메쉬 이력 카드(시간/통계/3D 그래프/프로빙 경로)

## 3.8 베드 평탄도 3D 그래프
**의도**: 숫자 테이블만으로는 파악하기 어려운 베드 기울기/굴곡을 직관적으로 확인

- 3D Heightmap(WebGL) 기반 표면 렌더링
- X/Y/Z 축 + 박스형 그리드 + 컬러 스케일
- 기준 평면(Flat)과 측정 표면(Probed) 동시 표시
- 프로빙 경로 START/END + serpentine 순서 오버레이
- 프린터 기준 방향 표기:
  - `X`: 좌 → 우
  - `Y`: 앞 → 뒤
  - `Z`: 높이 편차(mm)
...
### 8.4 자동레벨링이 진행되지 않음
- 베드 가열 단계에서 멈추면 히터 상태/온도 센서를 먼저 확인
- `TEMPERATURE_WAIT SENSOR=heater_bed MINIMUM=49` 단계 실패 시 Moonraker 로그 확인
- `SAVE_CONFIG` 단계에서 실패하면 Klipper 재시작/권한 문제 여부 점검

### 8.5 3D 그래프가 비거나 단순 수치만 보임
- 매트릭스가 `2x2` 미만(`1x1`)이면 표면 그래프를 만들 수 없음
- 모바일 브라우저 캐시가 오래된 경우 강력 새로고침으로 최신 번들 반영
