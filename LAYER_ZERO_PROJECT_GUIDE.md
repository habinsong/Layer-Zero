...
### 특징
- 기본은 경량 모드이며, `이 페이지 안에서 열기`를 눌렀을 때만 원본 UI iframe을 마운트
...
### 주요 기능
- Free/Paid 모드 전환
- 모델 선택(Free: Flash/Flash Lite, Paid: Flash/Flash Lite/Pro 3.1)
- 대화 이력 유지
- 최근 답변 복사/요약/재생성
- 모바일 폰트 크기 제어
...
- `aiFreeModel`, `aiPaidModel`, `aiUsageData`는 설정 동기화 대상에 포함
- API 키는 브라우저 로컬에만 암호화 저장되며 중앙 저장소에는 포함되지 않음
- 서버 연결 실패 시 로컬 폴백
- SSE로 다중 클라이언트 동기화
...
### 운영 포인트
- 목록에서는 히트맵만 렌더하고, `3D 보기`를 눌렀을 때만 Plotly 3D 그래프를 열어 메모리 사용을 절감
